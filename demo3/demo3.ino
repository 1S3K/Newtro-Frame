...
//  APDS.setSensitivity(99); //감도 조절 (1~100)
...
void equalizerMode(){
  for (int i = 0; i < 16; i++){ // 소리경계 설정
    ...
    for(int j=0;j<16;j++){
      if(i%2 == 1) pixelMap[j][15-i] = i*16 + j; // 정방향
      else pixelMap[j][15-i] = (i+1)*16 -j; // 역방향
      // 0~15 31~16 
    }
  }
  while (!Serial);
  ...
  do {
    ...
    for(int i=0;i<16;i++){ // i는 막대요
      for(int j=0;j<16;j++){ // j는 층이로다
        if(v[i] > VUlevelMap[j]) pixels.setPixelColor(pixelMap[i][j], pixels.Color(C[i][0], C[i][1], C[i][2])); // i인지 j인지 둘중하나.
...
void heartMode(){
  ...
  for(int i=0;i<3;i++){
    ...
    int baek = myBPM / 100;
    int sip = (myBPM-baek)/10;
    int il = myBPM-baek-sip;
    pixels.clear();
    pixels.setBrightness(BRIGHTNESS);
    setMiddleHeart();
    qor(baek);
